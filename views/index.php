...
<!--<a href="/customer/test">fdfff</a>-->
<? if (Tools::checkAccess('CUSTOMER', 'customer_create', 'edit')) : ?>
    <a id="add_new_blank" class="add_btn rc3 ddd" href="/customer/create" style="clear:both;">
        <span><img src="/static/img/plus_new.png" style="width: 10px; padding-top: 5px"></span>
        <span>Добавить заказчика<span class="ico"></span></span>
    </a>
<? endif; ?>
<?
...
foreach ($regions as $r) {
    ...
    /*if (!Yii::app()->user->regionId || $accessToAllRegions || ($r->id == 0 && $accessToNoRegions) ||  Yii::app()->user->regionId == $r->id) {
        if ($r->id != 0 || $accessToNoRegions) {
            $regionIds[] = $r->id;
        }
    }*/
}
...
// переадресация, для красивой ссылки
if ($region_id != -1 && (Yii::app()->request->getParam('region') == null || Yii::app()->request->getParam('region') != $region_id)) {
    $this->redirect('/customer/index/region/' . $region_id);
}
...
<? if (Tools::checkAccess('customer', 'birthDates', 'view')): ?>
    <div class='birthDates'>
        ...
        <div class="birthDaysButton">
            ...
            <!--        <a href='/customer/birthDates' id='fancy' style='text-decoration: none;' oncontextmenu="return false;">/customer/birthDates/region/$region_id;-->
            <!--           <img src='/static/img/mess_noread.png' alt='Даты рождения' class='birthDatesImg' title='Именинники'>-->
            <!--            <span class="green_btn" style="padding: 0px 27px 0px 27px; left: 18px;">-->
            <!--                Именинники-->
            <!--                --><? //if ($birthDayContacts['quantity']):?>
            <!--                    <i class="icountNotice ttl tooltipstered birthDaysQuantity">-->
            <? //=$birthDayContacts['quantity']?><!--</i>-->
            <!--                --><? //endif;?>
            <!--            </span>-->
            <!--        </a>-->
        </div>
                        ...
                        <? if (Tools::checkAccess('CUSTOMER', 'customer', 'edit') || Tools::checkAccess('CUSTOMER', 'customer', 'view')) : ?>
                            <li class="active">
                                <a onmouseover="this.style.color='#FFFFFF';" onmouseout="this.style.color='#131212';"
                                   style="color: #131212" href="/Customer/CustomerInArchive/">Архив</a>
                            </li>
                        <? endif; ?>
                    <? endif; ?>
                </ul>
...
<table style="width: 100%" id="customers_table" class="display dataTable no-footer">
    ...
    <?php foreach ($customers as $customer) { ?>
        ...
        if ($customer->in_archive != 1) {
            ...
            <tr class="tr_table" style="border-bottom: solid #b6b6b6 1px;" id="in_archive<?= $customer->id ?>">
                ...
                <? else: ?>
                    <td style="width: 100px">
                        ...
                        <? endif; ?>
                        <? if (Tools::checkAccess('CUSTOMER', 'customer', 'view')) : ?>
                            <a href="/customer/show/id/<?= $customer->id ?>" class="old-teg<?= $customer->id ?>"
                               style="text-decoration: none;" target="_blank">
                                <img width="35px" height="35px" class="ttl kr_img" src="/static/images/viev1.png"
                                     title="Просмотр" style="padding-left: 10px">
                            </a>
                        <? else: ?>
                            <img width="35px" height="35px" class="ttl kr_img" src="/static/images/viev1.png"
                                 title="Просмотр(Недоступно)" style="opacity: 0.25">
                        <? endif; ?>
                    </td>
                <? endif; ?>
            </tr>
